...
public final class ShellRuntime {
    ...
    // 客流计数器(RS485-2 = ttyS9)必须走原生 JHY 串口库(libjhy_serial_port.so)，对齐 V32 的 AppApplication.jhyopd。
    // 之前这里传 null，导致 useNativeJhyPort() 恒为 false、退回通用串口，ttyS9 始终打不开 → 客流计数器不工作。
