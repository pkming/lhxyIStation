...
final class LegacyCrc16 {
    ...
    static int calc(byte[] data, int offset, int len) {
        int ucCRCHi = 0;
        int ucCRCLo = 0;
        for (int i = 0; i < len; i++) {
            int index = (ucCRCLo ^ data[offset + i]) & 0x00FF;
            ucCRCLo = ucCRCHi ^ CRC16_TAB_H[index];
            ucCRCHi = CRC16_TAB_L[index];
        }
