...
public class Mesh extends BaseObj {
    ...
    void updateFromNative() {
        ...
        int[] iArr = new int[iNMeshGetVertexBufferCount];
        int[] iArr2 = new int[iNMeshGetIndexCount];
        int[] iArr3 = new int[iNMeshGetIndexCount];
        this.mRS.nMeshGetVertices(getID(this.mRS), iArr, iNMeshGetVertexBufferCount);
        this.mRS.nMeshGetIndices(getID(this.mRS), iArr2, iArr3, iNMeshGetIndexCount);
        ...
        for (int i = 0; i < iNMeshGetVertexBufferCount; i++) {
            if (iArr[i] != 0) {
                this.mVertexBuffers[i] = new Allocation(iArr[i], this.mRS, null, 1);
                this.mVertexBuffers[i].updateFromNative();
            }
        }
        for (int i2 = 0; i2 < iNMeshGetIndexCount; i2++) {
            if (iArr2[i2] != 0) {
                this.mIndexBuffers[i2] = new Allocation(iArr2[i2], this.mRS, null, 1);
                this.mIndexBuffers[i2].updateFromNative();
            }
            this.mPrimitives[i2] = Primitive.values()[iArr3[i2]];
        }
    }
    ...
    public static class Builder {
        ...
        public Mesh create() {
            ...
            this.mRS.validate();
            int[] iArr = new int[this.mVertexTypeCount];
            int[] iArr2 = new int[this.mIndexTypes.size()];
            int[] iArr3 = new int[this.mIndexTypes.size()];
            ...
            int i = 0;
            while (true) {
                Allocation allocationCreateSized2 = null;
                ...
                iArr[i] = allocationCreateSized2.getID(this.mRS);
                i++;
            }
            for (int i2 = 0; i2 < this.mIndexTypes.size(); i2++) {
                ...
                if (entry2.t != null) {
                    ...
                    allocationCreateSized = entry2.e != null ? Allocation.createSized(this.mRS, entry2.e, entry2.size, this.mUsage) : null;
                }
                int id = allocationCreateSized == null ? 0 : allocationCreateSized.getID(this.mRS);
                allocationArr2[i2] = allocationCreateSized;
                primitiveArr[i2] = entry2.prim;
                iArr2[i2] = id;
                iArr3[i2] = entry2.prim.mID;
            }
            Mesh mesh = new Mesh(this.mRS.nMeshCreate(iArr, iArr2, iArr3), this.mRS);
    ...
    public static class AllocationBuilder {
        ...
        public Mesh create() {
            ...
            for (int i = 0; i < this.mVertexTypeCount; i++) {
                ...
                iArr[i] = entry.a.getID(this.mRS);
            }
            for (int i2 = 0; i2 < this.mIndexTypes.size(); i2++) {
                Entry entry2 = (Entry) this.mIndexTypes.elementAt(i2);
                int id = entry2.a == null ? 0 : entry2.a.getID(this.mRS);
    ...
    public static class TriangleMeshBuilder {
        ...
        public Mesh create(boolean z) {
            ...
            if ((this.mFlags & 1) != 0) {
                builder.add(Element.F32_4(this.mRS), CalendarContract.ColorsColumns.COLOR);
            }
            if ((this.mFlags & 256) != 0) {
                builder.add(Element.F32_2(this.mRS), "texture0");
            }
            if ((this.mFlags & 2) != 0) {
                builder.add(Element.F32_3(this.mRS), SQLExec.DelimiterType.NORMAL);
            }
