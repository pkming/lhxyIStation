...
final class RealBufferedSource implements BufferedSource {
    ...
    public long read(Buffer buffer, long j) throws IOException {
        ...
        if (this.buffer.size == 0 && this.source.read(this.buffer, Trace.TRACE_TAG_RESOURCES) == -1) {
            return -1L;
        }
    ...
    public boolean exhausted() throws IOException {
        ...
        return this.buffer.exhausted() && this.source.read(this.buffer, Trace.TRACE_TAG_RESOURCES) == -1;
    }
    ...
    public boolean request(long j) throws IOException {
        ...
        while (this.buffer.size < j) {
            if (this.source.read(this.buffer, Trace.TRACE_TAG_RESOURCES) == -1) {
                return false;
            }
        }
        return true;
    ...
    public int select(Options options) throws IOException {
        ...
        } while (this.source.read(this.buffer, Trace.TRACE_TAG_RESOURCES) != -1);
        return -1;
    }
    ...
    public String readUtf8LineStrict() throws IOException {
        return readUtf8LineStrict(LinkQualityInfo.UNKNOWN_LONG);
    }
    ...
    public String readUtf8LineStrict(long j) throws IOException {
        ...
        long j2 = j == LinkQualityInfo.UNKNOWN_LONG ? Long.MAX_VALUE : j + 1;
        long jIndexOf = indexOf((byte) 10, 0L, j2);
        if (jIndexOf != -1) {
            return this.buffer.readUtf8Line(jIndexOf);
        }
        if (j2 < LinkQualityInfo.UNKNOWN_LONG && request(j2) && this.buffer.getByte(j2 - 1) == 13 && request(1 + j2) && this.buffer.getByte(j2) == 10) {
    ...
    public long indexOf(byte b) throws IOException {
        return indexOf(b, 0L, LinkQualityInfo.UNKNOWN_LONG);
    }

    @Override // okio.BufferedSource
    public long indexOf(byte b, long j) throws IOException {
        return indexOf(b, j, LinkQualityInfo.UNKNOWN_LONG);
    }
    ...
    public long indexOf(byte b, long j, long j2) throws IOException {
        ...
        while (j < j2) {
            ...
            if (j3 >= j2 || this.source.read(this.buffer, Trace.TRACE_TAG_RESOURCES) == -1) {
                break;
            }
            j = Math.max(j, j3);
        }
    ...
    public InputStream inputStream() {
        return new InputStream() { // from class: okio.RealBufferedSource.1
            ...
            public int read() throws IOException {
                ...
                if (RealBufferedSource.this.buffer.size == 0 && RealBufferedSource.this.source.read(RealBufferedSource.this.buffer, Trace.TRACE_TAG_RESOURCES) == -1) {
                    return -1;
                }
