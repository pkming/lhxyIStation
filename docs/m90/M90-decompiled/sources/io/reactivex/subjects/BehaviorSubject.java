...
public final class BehaviorSubject<T> extends Subject<T> {
    ...
    BehaviorDisposable<T>[] terminate(Object obj) {
        BehaviorDisposable<T>[] andSet = this.subscribers.get();
        BehaviorDisposable<T>[] behaviorDisposableArr = TERMINATED;
        if (andSet != behaviorDisposableArr && (andSet = this.subscribers.getAndSet(behaviorDisposableArr)) != behaviorDisposableArr) {
            setCurrent(obj);
        }
        return andSet;
    }
