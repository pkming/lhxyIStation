...
public final class FlowableTimeout<T, U, V> extends AbstractFlowableWithUpstream<T, T> {
    ...
    static final class TimeoutSubscriber<T> extends AtomicLong implements FlowableSubscriber<T>, Subscription, TimeoutSelectorSupport {
            ...
            if (j != LinkQualityInfo.UNKNOWN_LONG) {
                ...
                if (compareAndSet(j, j2)) {
                    ...
                    try {
                        ...
                        getAndSet(LinkQualityInfo.UNKNOWN_LONG);
        ...
        public void onError(Throwable th) {
            if (getAndSet(LinkQualityInfo.UNKNOWN_LONG) != LinkQualityInfo.UNKNOWN_LONG) {
                this.task.dispose();
                this.actual.onError(th);
            } else {
        ...
        public void onTimeout(long j) {
            if (compareAndSet(j, LinkQualityInfo.UNKNOWN_LONG)) {
    ...
    static final class TimeoutFallbackSubscriber<T> extends SubscriptionArbiter implements FlowableSubscriber<T>, TimeoutSelectorSupport {
            ...
            if (j != LinkQualityInfo.UNKNOWN_LONG) {
                ...
                if (this.index.compareAndSet(j, j2)) {
                    ...
                    try {
                        ...
                        Exceptions.throwIfFatal(th);
                        this.upstream.get().cancel();
                        this.index.getAndSet(LinkQualityInfo.UNKNOWN_LONG);
                        this.actual.onError(th);
                    }
                }
            }
        ...
        @Override // org.reactivestreams.Subscriber
        public void onError(Throwable th) {
            if (this.index.getAndSet(LinkQualityInfo.UNKNOWN_LONG) != LinkQualityInfo.UNKNOWN_LONG) {
                this.task.dispose();
                this.actual.onError(th);
                this.task.dispose();
                return;
        ...
        @Override // io.reactivex.internal.operators.flowable.FlowableTimeoutTimed.TimeoutSupport
        public void onTimeout(long j) {
            if (this.index.compareAndSet(j, LinkQualityInfo.UNKNOWN_LONG)) {
                SubscriptionHelper.cancel(this.upstream);
