...
public class POIFSReader {
    ...
    public void read(InputStream inputStream) throws IOException {
        ...
        processProperties(SmallBlockTableReader.getSmallDocumentBlocks(rawDataBlockList, propertyTable.getRoot(), headerBlockReader.getSBATStart()), rawDataBlockList, propertyTable.getRoot().getChildren(), new POIFSDocumentPath());
    }
        ...
        while (it.hasNext()) {
            ...
            if (property.isDirectory()) {
                ...
                if (listeners.hasNext()) {
                    ...
                    blockList.fetchBlocks(startBlock);
                } else {
                    blockList2.fetchBlocks(startBlock);
                }
            }
        }
    }
    ...
    private static class SampleListener implements POIFSReaderListener {
        ...
        public void processPOIFSReaderEvent(POIFSReaderEvent pOIFSReaderEvent) {
            ...
            try {
                ...
                stream.read(new byte[iAvailable]);
                int length = path.length();
                for (int i = 0; i < length; i++) {
                    System.out.print(new StringBuffer().append("/").append(path.getComponent(i)).toString());
                }
                System.out.println(new StringBuffer().append("/").append(name).append(": ").append(iAvailable).append(" bytes read").toString());
