...
public class Gcj extends DefaultCompilerAdapter {
    ...
    protected Commandline setupGCJCommand() {
        ...
        if (this.attributes.getSource() != null) {
            commandline.createArgument().setValue("-fsource=" + this.attributes.getSource());
        }
        if (this.attributes.getTarget() != null) {
            commandline.createArgument().setValue("-ftarget=" + this.attributes.getTarget());
        }
    ...
    public boolean isNativeBuild() {
        ...
        for (int i = 0; !zStartsWith && i < currentCompilerArgs.length; i++) {
            int i2 = 0;
            while (!zStartsWith) {
                String[] strArr = CONFLICT_WITH_DASH_C;
                if (i2 < strArr.length) {
                    zStartsWith = currentCompilerArgs[i].startsWith(strArr[i2]);
                    i2++;
                }
            }
        }
