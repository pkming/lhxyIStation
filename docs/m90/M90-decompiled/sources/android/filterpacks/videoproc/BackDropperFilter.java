...
public class BackDropperFilter extends Filter {
    ...
    private void allocateFrames(FrameFormat frameFormat, FilterContext filterContext) {
        if (createMemoryFormat(frameFormat)) {
            ...
            ShaderProgram shaderProgram = new ShaderProgram(filterContext, mSharedUtilShader + mBgDistanceShader);
            this.mBgDistProgram = shaderProgram;
            shaderProgram.setHostValue("subsample_level", Float.valueOf(this.mSubsampleLevel));
            ShaderProgram shaderProgram2 = new ShaderProgram(filterContext, mSharedUtilShader + mBgMaskShader);
            ...
            ShaderProgram shaderProgram3 = new ShaderProgram(filterContext, mSharedUtilShader + mUpdateBgModelMeanShader);
            this.mBgUpdateMeanProgram = shaderProgram3;
            shaderProgram3.setHostValue("subsample_level", Float.valueOf(this.mSubsampleLevel));
            ShaderProgram shaderProgram4 = new ShaderProgram(filterContext, mSharedUtilShader + mUpdateBgModelVarianceShader);
            ...
            ShaderProgram shaderProgram5 = new ShaderProgram(filterContext, mSharedUtilShader + mAutomaticWhiteBalance);
            ...
            ShaderProgram shaderProgram6 = new ShaderProgram(filterContext, mSharedUtilShader + mMaskVerifyShader);
    ...
    public void process(FilterContext filterContext) {
        ...
        if (this.mFrameCount <= this.mLearningDuration) {
            ...
            if (i2 == i3 - i4) {
                this.copyShaderProgram.process(this.mMask, this.mMaskVerify[z2 ? 1 : 0]);
                ...
                this.mMaskVerifyProgram.process(new Frame[]{gLFrameArr[i], this.mMask}, gLFrameArr[z2 ? 1 : 0]);
                this.mMaskVerify[z2 ? 1 : 0].generateMipMap();
                this.mMaskVerify[z2 ? 1 : 0].setTextureParameter(10241, 9985);
            }
            if (this.mFrameCount == this.mLearningDuration) {
                this.copyShaderProgram.process(this.mMaskVerify[z2 ? 1 : 0], this.mMaskAverage);
        ...
        if (this.mFrameCount < this.mLearningDuration - this.mLearningVerifyDuration || this.mAdaptRateBg > 0.0d || this.mAdaptRateFg > 0.0d) {
            ...
            this.mBgUpdateMeanProgram.process(new Frame[]{this.mVideoInput, gLFrameArr2[i], this.mMask}, gLFrameArr2[z2 ? 1 : 0]);
            this.mBgMean[z2 ? 1 : 0].generateMipMap();
            this.mBgMean[z2 ? 1 : 0].setTextureParameter(10241, 9985);
            GLFrame[] gLFrameArr3 = this.mBgVariance;
            this.mBgUpdateVarianceProgram.process(new Frame[]{this.mVideoInput, this.mBgMean[i], gLFrameArr3[i], this.mMask}, gLFrameArr3[z2 ? 1 : 0]);
            this.mBgVariance[z2 ? 1 : 0].generateMipMap();
            this.mBgVariance[z2 ? 1 : 0].setTextureParameter(10241, 9985);
        }
        if (this.mProvideDebugOutputs) {
    ...
    private void updateBgScaling(Frame frame, Frame frame2, boolean z) {
        ...
        if (width != this.mRelativeAspect || z) {
            ...
            if (this.mLogVerbose) {
                Log.v(TAG, "bgTransform: xMin, yMin, xWidth, yWidth : " + f4 + ", " + f2 + ", " + width + ", " + f3 + ", mRelAspRatio = " + this.mRelativeAspect);
            }
            this.mBgSubtractProgram.setHostValue("bg_fit_transform", new float[]{width, 0.0f, 0.0f, 0.0f, f3, 0.0f, f4, f2, 1.0f});
        }
    }
