...
class EventLogger {
    ...
    public void onWriteData() {
        ...
        if (this.mError || this.mPlaybackStartTime == -1 || this.mEngineCompleteTime == -1) {
            ...
            long j = this.mPlaybackStartTime - this.mReceivedTime;
            EventLogTags.writeTtsSpeakSuccess(this.mServiceApp, this.mCallerUid, this.mCallerPid, getUtteranceLength(), getLocaleString(), this.mRequest.getSpeechRate(), this.mRequest.getPitch(), this.mEngineStartTime - this.mRequestProcessingStartTime, this.mEngineCompleteTime - this.mRequestProcessingStartTime, j);
        }
    }
